...
// Implementation from "pairs" branch. Uses pairs { present: T[], deleted: number }
// instead of alternating []/number in the state array.

export class SparseArray<T> extends SparseItems<T[]> {
  static empty<T>(length = 0): SparseArray<T> {
    if (length === 0) return new this([], 0);
    else return new this([{ present: [], deleted: length }], length);
  }
  ...
  hasGet(index: number): [has: boolean, get: T | undefined] {
    ...
    if (i % 2 === 0) {
      return [true, this.state[i >> 1].present[offset]];
  ...
  *entries(): IterableIterator<[index: number, value: T]> {
    ...
    for (let i = 0; i < this.state.length; i++) {
      if (i % 2 === 0) {
        const present = this.state[i >> 1].present;
        for (const value of present) {
          yield [index, value];
          index++;
        }
      } else index += this.state[i >> 1].deleted;
    }
  }
  ...
  protected construct(state: Pair<T[]>[], length: number): this {
